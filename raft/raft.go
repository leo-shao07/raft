...
func (r *Raft) applyCommand(req *pb.ApplyCommandRequest) (*pb.ApplyCommandResponse, error) {
	...
	tmpLog := &pb.Entry{Id: lastId + 1, Term: r.currentTerm, Data: req.GetData()}
	r.appendLogs([]*pb.Entry{tmpLog})
	r.logger.Info("zzz")
	// TODO: (B.1)* - return the new log entry
	return &pb.ApplyCommandResponse{Entry: tmpLog}, nil
}
...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	if prevLogId != 0 && prevLogTerm != 0 {
		...
		tmp := r.getLog(prevLogId)
		if tmp == nil || tmp.Term != prevLogTerm {
			r.logger.Info("the given previous log from leader is missing or mismatched", zap.Uint64("prevLogId", prevLogId), zap.Uint64("prevLogTerm", prevLogTerm), zap.Uint64("logTerm", tmp.GetTerm()))
			return &pb.AppendEntriesResponse{Term: r.currentTerm, Success: false}, nil
		}
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	if lastTerm > req.LastLogTerm || (lastTerm == req.LastLogTerm && lastId > req.LastLogId) {
		// TODO: (A.7) - if votedFor is null or candidateId, and candidate’s log is at least as up-to-date as receiver’s log, grant vote
...
func (r *Raft) handleVoteResult(vote *voteResult, grantedVotes *int, votesNeeded int) {
	...
	if vote.RequestVoteResponse.GetTerm() > r.currentTerm {
		r.raftState.toFollower(vote.RequestVoteResponse.GetTerm())
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		tmpNextIndex := r.raftState.nextIndex[peerId]
		tmpEntries := r.getLogs(tmpNextIndex)
		prevEntry := r.getLog(tmpNextIndex - 1)
		req := &pb.AppendEntriesRequest{
			Term:           r.currentTerm,
			LeaderId:       r.id,
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	if result.GetTerm() > r.currentTerm {
		r.raftState.toFollower(result.Term)
	...
	if !result.GetSuccess() {
		...
		// r.setNextAndMatchIndex(result.peerId, r.nextIndex[result.peerId]+uint64(tmpLen), r.matchIndex[result.peerId]+uint64(tmpLen))
	...
	for i := len(logs) - 1; i >= 0; i-- {
		...
		replicas := 1
		tmp := (int)(r.commitIndex) + i + 1
		for _, j := range r.raftState.matchIndex {
			if (int)(j) >= tmp {
				replicas++
			}
		}
		...
		if replicas >= replicasNeeded {
			...
			// for k := int(r.commitIndex) + 1; k <= tmp; k++ {
			// 	ch <- r.getLog(uint64(k))
			// }
			r.raftState.commitIndex = uint64(tmp)
			r.applyLogs(r.applyCh)
